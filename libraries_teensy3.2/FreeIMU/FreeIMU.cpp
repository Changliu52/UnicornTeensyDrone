...
void FreeIMU::getQ(float * q, float * val) {
  ...
  q[3] = q3;
    //Serial.println(acc_scale_x);//!!!!!!!!!!
        ...
        ///////////////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////////////
        void FreeIMU::getQ_vis(float * q, float * val, float* qVision, HardwareSerial* XBee) {
            ...
            // gyro values are expressed in deg/sec, the * M_PI/180 will convert it to radians/sec
            MadgwickAHRSupdateIMU_vis(val[3] * M_PI/180, val[4] * M_PI/180, val[5] * M_PI/180, val[0], val[1], val[2], qVision, XBee);
...
const float def_sea_press = 1013.25;
	...
	*/
	float FreeIMU::getBaroAlt(float sea_press) {
        float temp = baro.getTemperature(MS561101BA_OSR_4096); //if(temp == NULL) return NULL; // Me added if
        float press = baro.getPressure(MS561101BA_OSR_4096); //if(press == NULL) return NULL; // Me added if
...
#define Kp1 0.55f                // PI observer velocity gain 0.55f
#define Kp2 1.0f                 // PI observer position gain 1.0f
#define Ki  0.001f               // PI observer integral gain (bias cancellation) 0.001f
        
        /**
         * Returns an altitude estimate from baromether fused with accelerometer readings
         */
        void FreeIMU::EstimatedAlt(float compensated_accel_earth, float* output) {
            static float EstAlt;
            static float EstVelocity;
            static float AltErrorI;
            static float lastTime;
            
            static float test;
            
            float baroAlt = getBaroAlt();
            ...
            if (baroAlt<10000000) {
                ...
                AltErrorI += AltError;
                AltErrorI = min(25000.0f, max(AltErrorI,-25000.0f));
                
                // Instant acceleration with integrated error correction
                float inst_acceleration = compensated_accel_earth * 9.80665f + AltErrorI * Ki;
                
                // Integrators with propotional error correction
                float dt = (currentTime - lastTime) / 1000000.0f;
                float Delta = inst_acceleration * dt + (Kp1 * dt) * AltError; //change of velocity
                EstAlt += (EstVelocity / 5.0f + Delta) * (dt / 2.0f) + (Kp2 * dt) * AltError; // Estimate Altitude
                EstVelocity += Delta * 10.0f;
                
                output[1] = EstAlt;
                output[0] = EstVelocity;
            }
...
void FreeIMU::getYpr_EarthAccel_EstAlt(float* ypr, float* dyn_acc_earth, float* output) {
    float q[4]; // quaternion
    float q2[4]; // quaternion
    float val[10];
    float gx, gy, gz; // estimated gravity direction
    ...
    gravityCompensateAcc(dyn_acc, q);
    
    // rotation to earth frame
    QuatRot(dyn_acc, q, dyn_acc_earth);
    for (int i = 1; i < 3; i++) {
        dyn_acc_earth[i] = min(0.09, max(-0.09, dyn_acc_earth[i]));
    }
    ...
    EstimatedAlt(dyn_acc_earth[3], output);
}
        ...
        void FreeIMU::getEuler_EarthAccel_EstAlt_gyro(float* euler, float* dyn_acc_earth, float* output, float* gyrorate) {
            float q[4]; // quaternion
            float q2[4]; // quaternion
            float val[10];
            float gx, gy, gz; // estimated gravity direction
            ...
            getQ(q, val);
            gyrorate[0] = val[3] * 3.1415927f/180.0f; // rad/s
            gyrorate[1] = val[4] * 3.1415927f/180.0f; // rad/s
            gyrorate[2] = val[5] * 3.1415927f/180.0f; // rad/s
        ...
        // Vision
        /////////////////////
        void FreeIMU::getEuler_EarthAccel_EstAlt_gyro_vis(float* q, float* dyn_acc_earth, float* output, float* gyrorate, float* qVision, HardwareSerial* XBee, int8_t vision_available_, float* biasAccel) {
            float q2[4]; // quaternion
            float val[10];
            float gx, gy, gz; // estimated gravity direction
            float dyn_acc[4];
            ...
            // remove vision estimated bias
            if (vision_available_ == 1){
                dyn_acc[1] += 0.1*biasAccel[0];
                dyn_acc[2] += 0.1*biasAccel[1];
                dyn_acc[3] += 0.1*biasAccel[2];
            }
            ...
            // accel bracket
            for (int i = 1; i < 4; i++) {
                dyn_acc_earth[i] = min(1.00, max(-1.00, dyn_acc_earth[i]));
            }
            
            // compute estimate altitude
            //EstimatedAlt(dyn_acc_earth[3], output);
            // copy the vision guard flag
            vision_available = vision_available_;
        }
        
        void FreeIMU::setExlong()
        {
            Exlong = ExlongDef;
        }
        /////////////////////////////////////////////
...
void gravityCompensateAcc(float * acc, float * q) {
  ...
  // compensate accelerometer readings with the expected direction of gravity(!!!!!!Bug fixed)
  acc[1] = acc[1] - g[0];
  acc[2] = acc[2] - g[1];
  acc[3] = acc[3] - g[2];
}
