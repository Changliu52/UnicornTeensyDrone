...
// temperature calibration curves. (3.31.14)
	...
	#define betaDef  0.1f//!!!!!!!!!!!to change0.05f
    #define ExlongDef 0.06f
#elif defined(GEN_MPU6050)
	#define twoKpDef  (2.0f * 0.5f)
...
class FreeIMU
{
  public:
	...
	////////////////////////
    // My function!!!!!!!!!!!!!!!!!!!!!!!
    void EstimatedAlt(float compensated_accel_earth, float* output);
    void getYpr_EarthAccel_EstAlt(float* ypr, float* dyn_acc_earth, float* output);
    void getEuler_EarthAccel_EstAlt_gyro(float* euler, float* dyn_acc_earth, float* output, float* gyrorate);
    void getEuler_EarthAccel_EstAlt_gyro_vis(float* euler, float* dyn_acc_earth, float* output, float* gyrorate, float* qVision, HardwareSerial* XBee, int8_t vision_available, float*biasAccel);
    void setExlong();
    bool initialised = 0; // set this to 0 will reset the altitude estimator
    /////////////////////
  ...
  private:
    ...
    float gxb, gyb, gzb, Exlong;
    int8_t vision_available;
    void MadgwickAHRSupdateIMU_vis(float gx, float gy, float gz, float ax, float ay, float az, float* qVision, HardwareSerial* XBee);
    void getQ_vis(float * q, float * val, float* qVision, HardwareSerial* XBee);
};
